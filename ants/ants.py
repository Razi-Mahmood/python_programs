...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        if exit: #check if the place1's exit (or exit) is not None
            exit.entrance = self
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                ...
                if hasattr(self.ant, 'is_container') and self.ant.is_container and self.ant.contained_ant is insect:
...
class HarvesterAnt(Ant):
    ...
    implemented = True
    # OVERRIDE CLASS ATTRIBUTES HERE
    food_cost = 2 #override the HarvesterAnt's food_cost with instructions' value
...
class ThrowerAnt(Ant):
    ...
    food_cost = 3 #override the ThrowerAnt's food_cost with instructions' value
    ...
    def nearest_bee(self, hive):
        ...
        near_bee = random_or_none(bee_place.bees)
        hop = 0
        while (bee_place is not hive) and ((near_bee is None and hop < self.max_range) or (hop <self.min_range)):
            bee_place = bee_place.entrance
            near_bee = random_or_none(bee_place.bees)
            hop += 1

        if bee_place is not hive and hop>=self.min_range and hop<=self.max_range:
            return near_bee
        else:
            return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        bee_list = list(place.bees)

        if (self.armor <= 0):

            for new_bee in bee_list:
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        "*** YOUR CODE HERE ***"
        self.armor = armor
        self.is_real_queen = QueenAnt.is_real_queen

        self.ant_list = []

        if (self.is_real_queen):
            QueenAnt.is_real_queen = False
    ...
    def action(self, colony):
        ...
        if (self.is_real_queen):
            ...
            while (current_place):
                if isinstance(current_place.ant, BodyguardAnt):

                    if (current_place.ant in self.ant_list):
                        if (not current_place.ant.contained_ant in self.ant_list):
                            current_place.ant.contained_ant.damage = 2 * current_place.ant.contained_ant.damage
                            self.ant_list.append(current_place.ant.contained_ant)

                    if (not current_place.ant in self.ant_list):
                        current_place.ant.damage = 2 * current_place.ant.damage
                        self.ant_list.append(current_place.ant)
                    ...
                    elif (not current_place.ant in self.ant_list) and current_place.ant.contained_ant in self.ant_list:
                        current_place.ant.damage = 2 * current_place.ant.damage #double the damage of the BodyguardAnt
                        self.ant_list.append(current_place.ant)

                    elif (current_place.ant in self.ant_list) and not current_place.ant.contained_ant in self.ant_list:
                        current_place.ant.contained_ant.damage = 2 * current_place.ant.contained_ant.damage
                        self.ant_list.append(current_place.ant.contained_ant)


                elif (current_place.ant and not (current_place.ant in self.ant_list)):
                    current_place.ant.damage = 2 * current_place.ant.damage
                    self.ant_list.append(current_place.ant)
