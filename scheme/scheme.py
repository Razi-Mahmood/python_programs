...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(expr):
        return env.lookup(expr) #finds the value of a name
    ...
    first, rest = expr.first, expr.second #first is operator, resr
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operator = scheme_eval(first, env) #extract the operator
        check_procedure(operator) #check if the operator is a procedure

        if isinstance(operator, MacroProcedure):
            return scheme_eval(operator.apply_macro(rest, env), env)


        helper = lambda x: scheme_eval(x, env) #create your 1-arg function
        operands = rest.map(helper) #rest is a Pair instance
        return scheme_apply(operator, operands, env) #applies a Scheme procedure to some arguments
...
def eval_all(expressions, env):
    ...
    while expressions.second is not nil: # ensures that the last element remains
        scheme_eval(expressions.first, env) #as we go through the list, we have to eval each expr
        expressions = expressions.second # moves through the list
...
#When and how do we create new Frames?
    #Whenever a user-defined procedure is called; we use the make_call_frame method of LambdaProcedure

    def __init__(self, parent):
        """An empty frame with parent frame PARENT (which may be None)."""
        self.bindings = {} #maps Scheme symbols (represented as Python strings) to Scheme values
        self.parent = parent #parent Frame instance
    ...
    def make_child_frame(self, formals, vals):
        ...
        new_frame_instance = Frame(self) #create a new Frame instance

        while formals and vals: # iterate through the formals and vals lists
            new_frame_instance.define(formals.first, vals.first) #call the define function to perform the bindings
            formals = formals.second #
            vals = vals.second

        if len(formals) == len(vals): #check the len of formals and vals to account for the SchemeError
            return new_frame_instance

        raise SchemeError("The number of argument values doesn't match with the number of formal parameters")
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            return self.fn(*python_args) #call self.fn using *python_args

        except TypeError: #if I get a TypeError Exception, the wrong number of arguments were passed
            raise SchemeError("The wrong number of arguments were passed: {0}".format(args))
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        new_frame = self.env.make_child_frame(self.formals, args) #create a new frame instance using self.env
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        vals = expressions.second.first #extract the value

        desired_val = scheme_eval(vals, env) #evaluate the values
        env.define(target, desired_val) #do the binding
        ...
        function_symbol = target.first #extract the 'f'
        new_lambda = LambdaProcedure(target.second, expressions.second, env) #call lambda to simplify

        env.define(function_symbol, new_lambda) #create binding btw f and the lambda

        return function_symbol #return f
...
def do_quote_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    return expressions.first #use .first to extract the operands in the Pair instance
...
def do_lambda_form(expressions, env):
    ...
    body = expressions.second #extract the body, the expression does NOT contain the special form keyword
...
def do_and_form(expressions, env):
    ...
    if expressions is nil: #if and is by itself, it is True
        return True

    while expressions.second is not nil:
        if scheme_falsep(scheme_eval(expressions.first, env)): #if I get a false, shortcircuit to False
            return False

        expressions = expressions.second #as we go through the list, we have to eval each expr

    return scheme_eval(expressions.first, env, True) #if no false values present at all, return the evaled true
...
def do_or_form(expressions, env):
    ...
    if expressions is nil: #if or is by itself, it is False
        return False

    while expressions.second is not nil:

        if scheme_truep(scheme_eval(expressions.first, env)): #if the 1st or case is true, shortcircuit to True
            return scheme_eval(expressions.first, env)

        expressions = expressions.second #as we go through the list, we have to eval each expr

    return scheme_eval(expressions.first, env, True) #if no true values present at all, return the evaled True
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if scheme_truep(test):
            ...
            if clause.second is nil: # if there is nothing to eval, default is the test (which is the rest)
                return test

            return eval_all(clause.second, env) #after the 1st true value,
                                            #   extract the 2nd part of the clause to evaluate
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil: #bindings is a list, so we will be iterating through it

        bind = bindings.first #extract the first binding
        #bindings = bindings.second #extract the value in a Pair instance

        check_form(bind, 2, 2) #a let binding has to have exactly 2 items

        symbol = bind.first #extract the symbol

        if formals is not nil and vals is not nil:
            formals = Pair(symbol, formals)  #create a Pair instance of the symbol
            vals = Pair(scheme_eval(bind.second.first, env), vals)
            #create Pair instance of evaled value

        if formals is nil:
            formals = Pair(symbol, nil)  #create a Pair instance of the symbol

        if vals is nil:
            vals = Pair(scheme_eval(bind.second.first, env), nil)

        bindings = bindings.second #extract the value in a Pair instance


    check_formals(formals) #checks that only symbols are being bound to #'s'
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        new_mu_frame = env.make_child_frame(self.formals, args) #create a new frame instance using self.env
        return new_mu_frame
